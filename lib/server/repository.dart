...
class Repository {
  ...
  Future<List<MovieModel>?> getMovieByGenereID(
    ...
    try {
      final response = await dio
          .get("${ConfigApi().getApiUrl()}/content_by_genre_id?id=$genereID&page=$pageNumber");
      if (response.statusCode == 200) {
        MovieListModel mobilistModel = MovieListModel.fromJson(response.data);
        return mobilistModel.movieList;
      }
  ...
  String password,{String? userId, String? reason}) async {
    dio.options.headers = ConfigApi().getHeaders();
    FormData formData = new FormData.fromMap(
  ...
  Future<bool?> saveChargeData(
    ...
    print("on save charge data methos");
    dio.options.headers = ConfigApi().getHeaders();
    FormData formData = new FormData.fromMap({
      "plan_id": planID,
      "user_id": userId,
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        appModeBox.put('isUserValidSubscriber', true).then((value) {
          print('Value saved: ${appModeBox.get('isUserValidSubscriber')}');
        });
        return true;
      }
    } catch (ex) {
  ...
  Future<bool> verifyMarketInApp(
    ...
    print("purchase data's are ${signedData}\n${signature}\n${publicKey}");
    dio.options.headers = ConfigApi().getHeaders();
    FormData formData = new FormData.fromMap({
