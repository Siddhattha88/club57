...
class SearchBloc extends Bloc<SearchEvent, SearchState> {
  ...
  SearchBloc(this.repository) : super(SearchInitailState()) {
    // Registering event handlers using `on` method
    on<GetSearchEvent>(_onGetSearchEvent);
  }

  Future<void> _onGetSearchEvent(
      GetSearchEvent event, Emitter<SearchState> emit) async {
    emit(SearchInitailState());
    try {
      final searchResultModel = await repository.searchResultData(
        query: event.param,
        type: "{movie,tvseries,tv_channels}",
      );
      emit(SearchIsLoaded(searchResultModel: searchResultModel));
    } catch (e) {
