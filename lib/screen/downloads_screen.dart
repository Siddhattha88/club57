            ...
            children: [
              ...
              itemCount: fileList.length,
                itemBuilder: (BuildContext context, int index) {
                  String fileName = p.basename(fileList[index].toString()); // Extract file name.
                  if(fileName.endsWith("\'") || fileName.endsWith("\"")){
                    fileName = fileName.substring(0, fileName.length - 1);
                  }
                  return InkWell(
                    onTap: () async {
                      File file = await _localFile(videoName: '/$fileName');
                      if (file.existsSync()) {
                        Navigator.push(
                          context,
                          MaterialPageRoute(
                            builder: (context) =>
                                MovieDetailsVideoPlayerWidget(localFile: file),
                          ),
                        );
                      } else {
