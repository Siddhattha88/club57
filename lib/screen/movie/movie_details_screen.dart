...
class _MovieDetailScreenState extends State<MovieDetailScreen> {
                        ...
                        Container(
                          ...
                          child: Column(
                            ...
                            children: [
                                ...
                                child: ElevatedButton(
                                  ...
                                  onPressed: () async {
                                    ...
                                    if (isUserValidSubscriber ||
                                      ...
                                      }else if(activeSubscription != null && activeSubscription!.status.toString().toLowerCase() =="active"){
                                        if (movieDetailsModel.videos!.length ==
                                            1) {
                                          Navigator.push(
                                              context,
                              ...
                              SizedBox(height: 10,),
                              //rent button
                              if (isDownloadEnable && movieDetailsModel.downloadLinks!.isNotEmpty)
                                Container(
                                  width:
                                      ...
                                      Navigator.pushNamed(
                                          ...
                                          arguments: {
                                            ...
                                            'videosID': allCommentModelList
                                                ...
                                                .elementAt(index)
                                                .videosId,
                                            'userId': authUser != null ? authUser.userId : "",
                                            'isDark': true,
                                          });
                                    },
                                    child: Padding(
  ...
  Widget favouriteMovie(String? userID, String? movieID) {
    return FutureBuilder<FavouriteResponseModel?>(
      future: Repository().verifyFavourite(userID, movieID),
      builder: (BuildContext context, AsyncSnapshot<FavouriteResponseModel?> snapshot) {
        if (snapshot.hasError) {
          return Text('Error: ${snapshot.error}', style: TextStyle(color: Colors.red),maxLines: 1,overflow: TextOverflow.ellipsis,);
        } else if (snapshot.hasData && snapshot.data != null) {
          final isFavourite = snapshot.data!.status == "success";
          return InkWell(
            onTap: () async {
              FavouriteResponseModel? responseModel;
              if (isFavourite) {
                responseModel = await Repository().removeFavourite(userID, movieID);
              } else {
                responseModel = await Repository().addFavourite(userID, movieID);
              }
              showShortToast(responseModel?.message ?? "");
              if (mounted) {
                setState(() {}); // Ensure FutureBuilder refreshes
              }
            },
            child: Icon(
              isFavourite ? Icons.favorite_outlined : Icons.favorite_border,
